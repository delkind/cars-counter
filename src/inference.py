...
from src.anchors import AnchorParameters, generate_anchors
...
def add_inference(
    ...
    """ Add inference layers to the training model.

    Args
        model                 : Training model to append bbox layers to.
    ...
    anchors = keras.layers.Concatenate(axis=1, name='anchors')([
        Anchors(
    ...
    regression = RegressBoxes(name='boxes')([anchors, regression])
    regression = ClipBoxes(name='clipped_boxes')([model.inputs[0], regression])
    ...
    # construct the model
    return keras.models.Model(inputs=model.inputs, outputs=detections)
